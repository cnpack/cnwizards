...
function FCI_FileOpen(lpPathName: PChar; Flag, Mode: Integer;
  ...
  {open a file}
begin
  Result := _lcreat(PAnsiChar(AnsiString(lpPathName)), 0);
...
var
  DT : Integer;
begin
  Result := _lopen(PAnsiChar(AnsiString(lpPathname)), OF_READ or OF_SHARE_DENY_NONE);
  if (Result = -1) then
    raise EAbFCIFileOpenError.Create;
...
function FDI_FileOpen(lpPathName: PChar; Flag, Mode: Integer) : Integer;
  ...
  {open a file}
begin
  Result := _lopen(PAnsiChar(AnsiString(lpPathName)), Mode);
end;
{ -------------------------------------------------------------------------- }
...
begin
  ...
  FH := _lopen(PAnsiChar(AnsiString(FP)), OF_READ or OF_SHARE_DENY_NONE);                       {!!.02}
