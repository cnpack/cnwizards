...
begin
  with Stream do begin
    ...
    // Here is FFileName is UnicodeString, it will lose data
    // Write( FFileName^, FFileNameLength );
    S := AnsiString(FFileName);
    Write(S[1], FFileNameLength);

    if FExtraFieldLength > 0 then
    begin
      S := AnsiString(FExtraField);
      Write(S[1], FExtraFieldLength);
    end;
  end;
end;
{ -------------------------------------------------------------------------- }
...
begin
  if Assigned( FItemInfo ) and ( FItemInfo.FileName <> nil ) then begin
    StrCopy(Buff, PAnsiChar(AnsiString(FItemInfo.FileName)));
{!!.03 - Added }
{$IFDEF Linux}
 { do nothing to Buff }
{$ELSE}
