inherited CnProjectBackupForm: TCnProjectBackupForm
  Left = 385
  Top = 274
  AutoScroll = False
  Caption = 'Project Backup'
  ClientHeight = 444
  ClientWidth = 653
  Icon.Data = {
    }
  KeyPreview = True
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object statMain: TStatusBar
    Left = 0
    Top = 425
    Width = 653
    Height = 19
    AutoHint = True
    Panels = <
      item
        Width = 200
      end
      item
        Width = 150
      end
      item
        Width = 150
      end>
    SimplePanel = False
  end
  object lvFileView: TListView
    Left = 0
    Top = 58
    Width = 653
    Height = 367
    Align = alClient
    Columns = <
      item
        Caption = 'Filename'
        Width = 150
      end
      item
        Caption = 'Project'
        Width = 100
      end
      item
        Caption = 'Path'
        Width = 300
      end
      item
        Alignment = taRightJustify
        Caption = 'Size(byte)'
        Width = 80
      end>
    MultiSelect = True
    ReadOnly = True
    RowSelect = True
    TabOrder = 2
    ViewStyle = vsReport
    OnCustomDrawItem = lvFileViewCustomDrawItem
    OnKeyDown = lvFileViewKeyDown
  end
  object tlbMain: TToolBar
    Left = 0
    Top = 0
    Width = 653
    Height = 31
    BorderWidth = 1
    Caption = 'tlbMain'
    EdgeBorders = [ebLeft, ebTop, ebRight, ebBottom]
    Flat = True
    Images = dmCnSharedImages.Images
    TabOrder = 0
    object btnAddFile: TToolButton
      Left = 0
      Top = 0
      Action = actAddFile
      ParentShowHint = False
      ShowHint = True
    end
    object btnAddOpened: TToolButton
      Left = 23
      Top = 0
      Action = actAddOpened
    end
    object btnAddDir: TToolButton
      Left = 46
      Top = 0
      Action = actAddDir
    end
    object btnRemoveFile: TToolButton
      Left = 69
      Top = 0
      Action = actRemoveFile
      ParentShowHint = False
      ShowHint = True
    end
    object btn1: TToolButton
      Left = 92
      Top = 0
      Width = 8
      ImageIndex = 1
      Style = tbsSeparator
    end
    object btnZip: TToolButton
      Left = 100
      Top = 0
      Action = actZip
    end
    object btnSprt1: TToolButton
      Left = 123
      Top = 0
      Width = 8
      ImageIndex = 16
      Style = tbsSeparator
    end
    object btnHelp: TToolButton
      Left = 131
      Top = 0
      Action = actHelp
    end
    object btnClose: TToolButton
      Left = 154
      Top = 0
      Action = actClose
    end
  end
  object pnlTool: TPanel
    Left = 0
    Top = 31
    Width = 653
    Height = 27
    Align = alTop
    BevelOuter = bvNone
    TabOrder = 1
    object lblProjects: TLabel
      Left = 8
      Top = 7
      Width = 38
      Height = 13
      Caption = '&Project:'
    end
    object lblLast: TLabel
      Left = 296
      Top = 7
      Width = 3
      Height = 13
    end
    object cbbProjectList: TComboBox
      Left = 82
      Top = 3
      Width = 175
      Height = 21
      Style = csDropDownList
      ItemHeight = 13
      TabOrder = 0
      OnChange = cbbProjectListChange
    end
  end
  object actlstMain: TActionList
    Images = dmCnSharedImages.Images
    Left = 24
    Top = 88
    object actAddFile: TAction
      Caption = 'Add Files...'
      Hint = 'Add Files to List'
      ImageIndex = 14
      OnExecute = actAddFileExecute
    end
    object actRemoveFile: TAction
      Caption = 'Remove Selected Files'
      Hint = 'Remove Selected Files from List'
      ImageIndex = 15
      OnExecute = actRemoveFileExecute
    end
    object actZip: TAction
      Caption = 'Compress and Save'
      Hint = 'Compress and Save the Backup File'
      ImageIndex = 6
      OnExecute = actZipExecute
    end
    object actHelp: TAction
      Caption = 'Help'
      Hint = 'Show Help Information'
      ImageIndex = 1
      OnExecute = actHelpExecute
    end
    object actClose: TAction
      Caption = 'Exit'
      Hint = 'Close this Window and Exit'
      ImageIndex = 0
      OnExecute = actCloseExecute
    end
    object actAddOpened: TAction
      Caption = 'Add All Opened Files'
      Hint = 'Add All Opened Files in IDE'
      ImageIndex = 95
      OnExecute = actAddOpenedExecute
    end
    object actAddDir: TAction
      Caption = 'Add From Directory...'
      Hint = 'Add All Files From a Directory Tree'
      ImageIndex = 96
      OnExecute = actAddDirExecute
    end
  end
  object dlgOpen: TOpenDialog
    Filter = 
      'Project Files (*.pas,*.inc,*.dfm,*.xfm,*.cpp,*.h,*.hpp,*.res,*.d' +
      'pr,*.dpk,*.bpr,*.bpg)|*.pas;*.inc;*.dfm;*.xfm;*.cpp;*.h;*.hpp;*.' +
      'res;*.dpr;*.dpk;*.bpr;*.bpg|Documents (*.doc,*.wps,*.xls,*.txt,*' +
      '.pdf)|*.doc,*.wps,*.xls,*.txt,*.pdf|All Files (*.*)|*.*'
    Options = [ofAllowMultiSelect, ofPathMustExist, ofFileMustExist, ofEnableSizing]
    Left = 64
    Top = 88
  end
end
