...
用例先按存在形式分 Delphi 平台下的用例、Lazarus 平台下的用例、脚本用例三大类。

其中前两个平台下的用例又分菜单专家形式、和独立应用程序形式两种。
...
2. 脚本形式，统一放在 Script 子目录下。0.8.2 以及后续版本的 CnWizards 中支持 Pascal Script 脚本。CnWizards 自身开放了部分接口可与脚本交互，因此可以通过脚本的方式来进行 CnWizards 的功能测试。具体实例可参考Bin\PSDemo下的文件。
脚本形式的测试用例，源码文件命名推荐 CnTest 开头，Script 结束，如 CnTestXXXScript。

3.独立应用程序形式。每个用例须在各自所在平台的目录下建立一单独目录。测试执行时直接编译目录内的工程文件即可。该类用例主要用来测试一些和 IDE 关联性不强的工具函数库。
